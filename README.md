...
| `--reuse_scene`      | `'store_true'` | Setting whether to reuse previously initialized scene or not. The initialized scene should exist in `initialized/{MODEL_IDX}-{SCENE_IDX}.json`. If there is none, the system will set this flag `False` and start the initialization of the scene from the beginning. | -                 |
